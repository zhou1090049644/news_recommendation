...
from utils import Logger, evaluate, gen_sub, get_mode_dir, is_eval_mode
...
def train_model(df_feature, df_query, model_path):
    ...
    for fold_id, (trn_idx, val_idx) in enumerate(
        ...
        joblib.dump(model, f'{model_path}/lgb{fold_id}.pkl')
    ...
    if has_test:
        ...
        log.info('df_test is empty, skip submission prediction')


def online_predict(df_test, model_path):
    ycol = 'label'
    feature_names = list(
        filter(
            lambda x: x not in [ycol, 'created_at_datetime', 'click_datetime'],
    ...
    for fold_id in tqdm(range(5)):
        model_file = f'{model_path}/lgb{fold_id}.pkl'
        if not os.path.exists(model_file):
            model_file = f'../user_data/model/lgb{fold_id}.pkl'
...
if __name__ == '__main__':
    ...
    if is_eval_mode(mode):
        df_feature = pd.read_pickle(f'../user_data/data/{mode_dir}/feature.pkl')
        ...
        train_model(df_feature, df_query, model_path)
    else:
        df_feature = pd.read_pickle(f'../user_data/data/{mode_dir}/feature.pkl')
        online_predict(df_feature, model_path)
