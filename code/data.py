...
def data_offline(df_train_click, df_test_click):
    ...
    # Alternative: sample exactly 50000 unique users.
    # train_users = df_train_click['user_id'].unique().tolist()
    # val_users = set(sample(train_users, min(50000, len(train_users))))
...
def data_local(df_train_click, df_test_click):
    df_test_click = df_test_click.sort_values(
        ['user_id', 'click_timestamp']).reset_index(drop=True)

    click_list = [df_train_click]
    query_list = []

    groups = df_test_click.groupby(['user_id'])
    for _, g in tqdm(groups):
        query = g.tail(1)
        query_list.append(query[['user_id', 'click_article_id']])

        history = g.head(g.shape[0] - 1)
        if history.shape[0] > 0:
            click_list.append(history)

    df_click = pd.concat(click_list, sort=False).reset_index(drop=True)
    df_query = pd.concat(query_list, sort=False).reset_index(drop=True)
    ...
    os.makedirs('../user_data/data/local', exist_ok=True)

    df_click.to_pickle('../user_data/data/local/click.pkl')
    df_query.to_pickle('../user_data/data/local/query.pkl')
...
if __name__ == '__main__':
    ...
    elif mode == 'testa_valid':
        data_local(df_train_click, df_test_click)
    else:
